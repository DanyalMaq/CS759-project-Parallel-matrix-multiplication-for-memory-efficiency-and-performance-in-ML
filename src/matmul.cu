...
__global__ void matmul_rect(float *A, float *B, float *C,
    ...
    __shared__ float sB[TILE_WIDTH][TILE_WIDTH];    // No bank conflict since sB[ty][tx] accesses consecutive banks with stride 1
    int bx = blockIdx.x;
    int by = blockIdx.y;
    int tx = threadIdx.x;
    int ty = threadIdx.y;
...
__global__ void matmul_rect_relu_(float *A, float *B, float *C,
                                     int nRowsA, int nColsA, int nColsB
                                    ) {
    __shared__ float sA[TILE_WIDTH][TILE_WIDTH];   // Tile size of 32x32
